...
import { formatMessageContext, calcExpiry, toMiliseconds, generateRandomBytes32, hasOverlap, isSignalTypePairing, isSequenceFailed, isSequenceResponded, isStoreUpdatedEvent, ERROR, isSequenceRejected, } from "@walletconnect/utils";
import { formatJsonRpcError, formatJsonRpcRequest, formatJsonRpcResult, isJsonRpcError, isJsonRpcRequest, isJsonRpcResponse, } from "@walletconnect/jsonrpc-utils";
import { STORE_EVENTS, EXPIRER_EVENTS, RELAYER_DEFAULT_PROTOCOL, FIVE_MINUTES, THIRTY_SECONDS, ONE_DAY, RELAYER_EVENTS, } from "../constants";
export class Engine extends IEngine {
    ...
    async send(topic, payload, chainId) {
        ...
        if (isJsonRpcResponse(originalPayload) ||
    ...
    create(params) {
        return new Promise(async (resolve, reject) => {
            ...
            const maxTimeout = (params === null || params === void 0 ? void 0 : params.timeout) || toMiliseconds(FIVE_MINUTES);
            ...
            this.sequence.pending.on(STORE_EVENTS.updated, async (updatedEvent) => {
    ...
    async onResponse(payloadEvent) {
        ...
        if (!isSequenceRejected(response)) {
            ...
            await this.sequence.client.relayer.publish(pending.topic, typeof error === "undefined"
    ...
    async onExtension(payloadEvent) {
        const { topic, payload } = payloadEvent;
    ...
    async onNotification(payloadEvent) {
    ...
    async handleExtension(topic, extension, participant) {
    ...
    async onPendingStatusEvent(event) {
        ...
        if (isSequenceResponded(pending)) {
            ...
            if (!isStoreUpdatedEvent(event)) {
                ...
                const params = !isSequenceFailed(outcome)
                    ? {
                        relay: outcome.relay,
    ...
    async subscribeNewPending(createdEvent) {
    ...
    async subscribeNewSettled(createdEvent) {
    ...
    registerEventListeners() {
        this.sequence.pending.on(STORE_EVENTS.created, async (createdEvent) => {
            await this.subscribeNewPending(createdEvent);
            await this.onPendingStatusEvent(createdEvent);
        });
        this.sequence.pending.on(STORE_EVENTS.updated, async (updatedEvent) => await this.onPendingStatusEvent(updatedEvent));
        this.sequence.pending.on(STORE_EVENTS.deleted, async (deletedEvent) => {
        ...
        this.sequence.settled.on(STORE_EVENTS.created, async (createdEvent) => {
        ...
        this.sequence.settled.on(STORE_EVENTS.updated, async (updatedEvent) => {
        ...
        this.sequence.settled.on(STORE_EVENTS.deleted, async (deletedEvent) => {
        ...
        this.sequence.settled.on(STORE_EVENTS.sync, () => this.sequence.events.emit(this.sequence.config.events.sync));
        this.sequence.client.relayer.on(RELAYER_EVENTS.payload, (payloadEvent) => {
        ...
        this.sequence.expirer.on(EXPIRER_EVENTS.expired, async (expiredEvent) => {
